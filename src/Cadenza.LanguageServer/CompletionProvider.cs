...
public class CompletionProvider
{
    ...
    /// </summary>
    public CompletionList? GetCompletions(ManagedDocument document, LspPosition position)
    {
    ...
    /// <summary>
    /// Compatibility method for tests - returns string array instead of CompletionList
    /// </summary>
    public string[] GetCompletions(ManagedDocument document, int line, int column)
    {
        var position = new LspPosition(line, column);
        var completionList = GetCompletions(document, position);
        return completionList?.Items?.Select(item => item.Label).ToArray() ?? new string[0];
    }

    /// <summary>
    /// Analyze the context around the completion position
    /// </summary>
    private CompletionContext AnalyzeCompletionContext(ManagedDocument document, LspPosition position)
    {
