...
public class StaticAnalyzer
{
    ...
    /// <summary>
    /// Analyze a single FlowLang file
    /// </summary>
    ...
    /// <summary>
    /// Analyze FlowLang source code from string
    /// </summary>
    public AnalysisReport AnalyzeFile(string filePath, string sourceText)
    {
        try
        {
            // Parse the FlowLang source
            var lexer = new FlowLangLexer(sourceText);
    ...
    {
        if (!filePath.EndsWith(".flow") || _configuration.ShouldExcludeFile(filePath))
        {
            return;
        }
    ...
    {
        var flowFiles = Directory.GetFiles(directoryPath, "*.flow", SearchOption.AllDirectories);
    ...
    {
        Console.WriteLine();
        Console.WriteLine("=== FlowLang Static Analysis Report ===");
        Console.WriteLine();
