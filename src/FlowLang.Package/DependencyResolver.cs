...
public class DependencyResolver
{
    ...
    private async Task<ResolvedPackage?> ResolvePackage(string packageName, string versionSpec)
    {
            ...
            if (metadata != null)
            {
                ...
                if (version != null)
                {
                    return new ResolvedPackage(
                        Version: version,
                        Resolved: $"https://packages.flowlang.org/{packageName}/-/{packageName}-{version}.zip",
        ...
        catch (Exception ex)
        {
            ...
            Console.WriteLine($"Warning: Failed to resolve {packageName} from FlowLang registry: {ex.Message}");
        }
