...
public class PackageManager
{
    ...
    public PackageManager(EnhancedFlowcConfig? config = null)
    {
        ...
        _registry = new FlowLangRegistryClient(config.FlowLangRegistry);
    ...
    public async Task<List<PackageSearchResult>> SearchPackagesAsync(string query)
    {
        ...
        try
        {
            ...
            results.AddRange(flowLangPackages.Select(p => new PackageSearchResult(
                p.Name, p.Version, p.Description, PackageType.FlowLang, p.DownloadCount)));
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Warning: FlowLang registry search failed: {ex.Message}");
        }
    ...
    public async Task<PackageInfo?> GetPackageInfoAsync(string packageName)
    {
        ...
        try
        {
            ...
            if (metadata != null)
            {
                ...
                return new PackageInfo(
                    ...
                    PackageType.FlowLang, metadata.Keywords, versions,
        ...
        {
            Console.WriteLine($"Warning: Failed to get FlowLang package info: {ex.Message}");
        }
    ...
    /// <summary>
    /// Publish a package to the FlowLang registry
    /// </summary>
    ...
    private async Task InstallSinglePackage(string packageName, ResolvedPackage package, string packagesDir)
    {
        ...
        if (package.Type == PackageType.FlowLang)
        {
            await InstallFlowLangPackage(packageName, package, packageDir);
        }
    ...
    private async Task InstallFlowLangPackage(string packageName, ResolvedPackage package, string packageDir)
    {
    ...
    private async Task InstallNuGetPackage(string packageName, ResolvedPackage package, string packageDir)
    {
        ...
        var bindingsPath = Path.Combine(packageDir, $"{packageName}.flow");
    ...
    private async Task<ResolvedPackage?> ResolvePackageSpec(string packageName, string versionSpec)
    {
        ...
        if (metadata != null)
        {
            return new ResolvedPackage(
                ...
                Resolved: $"https://packages.flowlang.org/{packageName}/-/{packageName}-{metadata.Version}.tgz",
                Integrity: "",
