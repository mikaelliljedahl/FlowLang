...
/// <summary>
/// Enhanced FlowLang project configuration with comprehensive package management support
/// </summary>
public record EnhancedFlowcConfig(
    ...
    string FlowLangRegistry = "https://packages.flowlang.org",
...
public record PublishConfig(
    string Registry = "https://packages.flowlang.org",
...
public record ResolvedPackage(
    ...
    PackageType Type = PackageType.FlowLang
)
{
    public Dictionary<string, string> Dependencies { get; init; } = Dependencies ?? new();
...
/// <summary>
/// Package metadata for FlowLang registry
/// </summary>
...
public static class ConfigurationManager
{
    ...
    public static async Task<EnhancedFlowcConfig> LoadConfigAsync(string? path = null)
    {
        ...
        {
            throw new InvalidOperationException($"Invalid flowc.json format: {ex.Message}", ex);
        }
    }
    ...
    public static async Task<LockFile> LoadLockFileAsync(string? path = null)
    {
        ...
        {
            throw new InvalidOperationException($"Invalid flowc.lock format: {ex.Message}", ex);
        }
    }
    ...
    {
        var configPath = Path.Combine(path, "flowc.json");
    ...
    {
        var config = await LoadConfigAsync(Path.Combine(rootPath, "flowc.json"));
        ...
        foreach (var projectPattern in config.Workspace.Projects)
        {
            ...
            {
                if (File.Exists(Path.Combine(match, "flowc.json")) && 
